...
def create_input(client: EquiaClient):
    argument = client.get_batchflash_fixed_temperature_pressure_input()
...
def print_value(argument):
    print(argument.ljust(35), end="", flush=True)
...
async def call_batchflash():
    client = create_client()

    argument = create_input(client)

    result = await client.call_batchflash_fixed_temperature_pressure_async(argument)
    ...
    if (isinstance(result, ProblemDetails)):
        ...
        print_calculation_result(result.points[0])
        print("")
        print_calculation_result(result.points[1])
    else:
        print_exception_info(result.exception_info)

loop = asyncio.get_event_loop()
loop.run_until_complete(call_batchflash())
